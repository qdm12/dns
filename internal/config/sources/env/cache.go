...
func (r *Reader) getLRUCacheMaxEntries() (maxEntries int, err error) {
	...
	maxEntriesInt64, err := strconv.ParseInt(s, base, bits)
	switch {
	case err != nil:
		return 0, fmt.Errorf("environment variable CACHE_LRU_MAX_ENTRIES: %w", err)
	case maxEntriesInt64 < 0:
		return 0, fmt.Errorf("%w: cannot be negative", ErrCacheLRUMaxEntries)
	case maxEntriesInt64 == 0:
		return 0, fmt.Errorf("%w: cannot be zero", ErrCacheLRUMaxEntries)
	case maxEntriesInt64 > math.MaxInt:
	...
	maxEntries = int(maxEntriesInt64)
