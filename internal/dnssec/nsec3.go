...
func extractNSEC3s(rrSets []dnssecRRSet) (rrs []dns.RR) {
...
func nsec3ValidateNoDataDS(qname, delegationName string,
	...
	if !optOutBitSet {
		return fmt.Errorf("for qname %s: %w: "+
			...
			qname, errNSEC3NoDataDSOptOutNotSet, nextCloser, nextCloserCoveringNSEC3.Flags)
	}
...
// nsec3ValidateWildcard validates a positive response generated by
// wildcard expansion. The existence of the generating wildcard is
// proven by the verified wildcard RRSIG of the answer RRSet, so per
// RFC 5155 section 8.8 the only additional proof required is an NSEC3
// covering the "next closer" name, which is qname itself since a
// wildcard only matches a name one label below its closest encloser.
...
func nsec3ValidateWildcard(qname string, nsec3RRSet []dns.RR) (err error) {
	nextCloserCoveringNSEC3 := nsec3FindCovering(qname, nsec3RRSet)
	if nextCloserCoveringNSEC3 != nil {
		return nil
	}
